...
class CycleGANModel(BaseModel):
    ...
    def modify_cmd_options(parser, is_train=True):
        ...
        For CycleGAN, in addition to GAN losses, we introduce lambda_A, lambda_B, and lambda_idt for the following losses.
        A (source domain), B (target domain).
        Generators: G_A2B: A -> B; G_B2A: B -> A.
        Discriminators: D_A: G_A2B(A) vs. B; D_B: G_B2A(B) vs. A.
        Forward cycle loss:  lambda_A * ||G_B2A(G_A2B(A)) - A|| (Eqn. (2) in the paper)
        Backward cycle loss: lambda_B * ||G_A2B(G_B2A(B)) - B|| (Eqn. (2) in the paper)
        Identity loss (optional): lambda_idt * (||G_A2B(B) - B|| * lambda_B + ||G_B2A(A) - A|| * lambda_A) (Sec 5.2 "Photo generation from paintings" in the paper)
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_idt', type=float, default=0.5, help='use identity mapping. Setting lambda_idt other than 0 has an effect of scaling the weight of the identity mapping loss. For example, if the weight of the identity loss should be 10 times smaller than the weight of the reconstruction loss, please set lambda_idt = 0.1')
    ...
    def __init__(self, opt):
        ...
        if self.is_train and self.opt.lambda_idt > 0.0:  # if identity loss is used, we also visualize idt_B=G_A2B(B) ad idt_A=G_A2B(B)
        ...
        # Code (vs. paper): G_A2B (G), G_B2A (F), D_A (D_Y), D_B (D_X)
        ...
        if self.is_train:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A2B.parameters(), self.netG_B2A.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_G(self):
        ...
        self.loss_G_A2B = self.criterionGAN(self.netD_A(self.fake_B), True)
        # GAN loss D_B(G_B2A(B))
        self.loss_G_B2A = self.criterionGAN(self.netD_B(self.fake_A), True)
